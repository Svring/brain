...
interface UpdateLaunchpadToolCallMessageProps {
	...
	image?: string;
	cpu?: LaunchpadCpuOption;
	memory?: LaunchpadMemoryOption;
...
export function UpdateLaunchpadToolCallMessage({
	...
	const cpuIndex =
		currentCpu !== undefined
			? LAUNCHPAD_CPU_OPTIONS.findIndex(
					(option: LaunchpadCpuOption) => option === currentCpu,
				)
			: -1;
	const memoryIndex =
		currentMemory !== undefined
			? LAUNCHPAD_MEMORY_OPTIONS.findIndex(
					(option: LaunchpadMemoryOption) => option === currentMemory,
				)
			: -1;
