...
import { Loader2, AlertCircle, CheckCircle, Bot } from "lucide-react";
...
export const ResourceLog: React.FC<ResourceLogProps> = ({ payload }) => {
  ...
  React.useEffect(() => {
    if (logsData) {
      console.log("Resource logs result:", logsData);
    }
    if (error) {
      console.error("Resource logs error:", error);
    }
  }, [logsData, error]);
  ...
  // Handle cluster resources (object with supported property)
  if (payload.type === "custom" && payload.resourceType === "cluster") {
    if (
      !logsData ||
  ...
  // Handle launchpad resources (array)
