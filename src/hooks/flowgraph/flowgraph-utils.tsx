...
export const convertReliancesToEdges = (reliances: ResourceReliances, resourceObjects?: any[]): Edge[] =>
    ...
    _.flatMap(Object.entries(ownerResources), ([ownerName, dependencies]) => {
      ...
      return _.map(dependencies, (dependency) => {
        const isDevboxToLaunchpad = 
          ...
          (ownerKind.toLowerCase() === "deployment" || ownerKind.toLowerCase() === "statefulset");

        let devboxVersion = null;
        if (isDevboxToLaunchpad && resourceObjects) {
          const launchpadResource = resourceObjects.find(
            (resource) => 
              (resource.kind?.toLowerCase() === "deployment" || resource.kind?.toLowerCase() === "statefulset") && 
          ...
          } else if (launchpadResource?.spec?.template?.spec?.containers?.[0]?.image) {
        ...
        const edgeData = isDevboxToLaunchpad && devboxVersion ? { devboxVersion } : undefined;
