...
interface TrialWithQueryProps {
	query: string;
}

export function TrialWithQuery({ query }: TrialWithQueryProps) {
	const {
		submitWithContext,
		isLoading,
		messages,
	...
	const handleRegisterClick = () => {
		...
		if (inputValue.trim()) {
			queryString += `&followup=${encodeURIComponent(inputValue.trim())}`;
		}
				...
				<>
					...
					<div className="flex-1 flex flex-col min-h-0">
						<button
							...
							type="button"
						>
							<div className="max-w-3xl mx-auto w-full pointer-events-none h-full">
