...
export const QueryLogsRequestSchema = z.object({
  time: z.string(), // 查询的时间范围，例如：1h, 1m, 1d
  namespace: z.string(), // 查询的命名空间名称
  app: z.string(), // 查询的app
  limit: z.string(), // 返回的日志条数限制，默认值为 10
  jsonMode: z.string(), // 是否开启Json模式 true则开启，false则不开启
  stderrMode: z.string(), // 是否只查看stderr，true则开启只查看，false则不开启只查看
  numberMode: z.string(), // 是否查询日志条目，true则查看，false则不查看
  numberLevel: z.string().optional(), // 查询日志条目的层级，值为m，h，d等
  pod: z.array(z.string()), // 需要查询的Pod，传空则查询全部Pod
  container: z.array(z.string()), // 需要查询的Container，传空则查询全部Container
  keyword: z.string(), // 查询时过滤的关键词
  jsonQuery: z.array(JsonQuerySchema), // 使用json模式时的查询参数
});
