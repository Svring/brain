...
export const transformIngressResources = (
  ...
  resources.forEach((resource) => {
    ...
    const fixedDomain = resource.metadata.labels?.[
        ...
        if (rule.http?.paths && Array.isArray(rule.http.paths)) {
          rule.http.paths.forEach((path) => {
            if (path.backend?.service?.port?.number) {
              ...
              const customDomain = host && host !== fixedDomain ? host : fixedDomain;
...
export const composeAddressFromIngress = (
  ...
  return ports.map((port) => {
    ...
    const publicAddress = domainForPublicAddress ? `${protocol}s://${domainForPublicAddress}` : undefined;
